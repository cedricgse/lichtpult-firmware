...
uint8_t rxbuffer[buffer_size];         // Buffer to write data received from the master
uint8_t buffer_adr = 0; 					// Virtual buffer address register
uint8_t buffer_read_adr = 0;
...
//######################################## Master Read Data Mode - Slave receive
		...
		case USI_SLAVE_GET_DATA_AND_SEND_ACK:
			...
			*/
			...
			buffer_adr++;
			if(buffer_adr >= buffer_size) buffer_adr = 0;
...
{
	uint8_t temp_data = rxbuffer[buffer_read_adr];
	buffer_read_adr++;
	if(buffer_read_adr >= buffer_size) buffer_read_adr = 0;
	return temp_data;
}

uint8_t usiTwiDataInReceiveBuffer()
{
	if(buffer_adr == buffer_read_adr) return 0;
	return 1;
}